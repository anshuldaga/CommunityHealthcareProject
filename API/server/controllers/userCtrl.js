...
exports.createUser = (req, res, next) => {
  ...
  var checkUsernameSql =
    'SELECT COUNT(*) AS count FROM usercredentials WHERE username = ?;';
  mysqlConnection.query(checkUsernameSql, [user.username], (err, rows) => {
    if (err) {
      ...
      if (rows[0].count > 0) {
        ...
        var sql =
          'INSERT INTO usercredentials(firstName, lastName, address, username, password, lastUpdatedDT) VALUES (?, ?, ?, ?, ?, now());';
